...
ui <- fluidPage(
        ...
        titlePanel("Alternative Effect Size Calculator", windowTitle = "ShinyAESC"),
        ...
        sidebarLayout(
                ...
                sidebarPanel(
                        ...
                        fileInput('file1', 'Upload Data (*.csv and less than 30mgb)',
                ...
                mainPanel(
                        tabsetPanel(
                                ...
                                tabPanel("Raw Data", h3("Raw Data"),tableOutput('contents')),
                                ...
                                tabPanel("Traditional Statistics", 
                                         h3("Descriptive Statistics"),
                                         tableOutput('descriptable'), 
                                         plotOutput("histogram.X"), 
                                         plotOutput("histogram.Y"), 
                                         plotOutput('corplot'),
                                        h3("Traditional Effect Size Indices"),
                                        tableOutput("cles")),
                                tabPanel("Alternative Effect Sizes",
                                         
                                         h3("Alternative Effect Sizes"),
                                         h4("Common language effect size"),
                                         textOutput("cles.verbal"),
                                         plotOutput("histogram.overlap"), 
                                         h5("Descriptive Statistics separated by predictor"),
                                         tableOutput("clestable"),
                                         hr(),
                                         h4("Binomial effect size display (BESD)"),
                                         tableOutput('besd')
                                         ),
                                tabPanel("Help", fluidPage(
...
server <- function(input, output, session) {
        ...
        cutoff_X <- reactive({
                ...
                exp.X <- select(df, var.X)
                co.X <- input$cutoff.X
                cutoff_X <- as.numeric(quantile(exp.X, probs = co.X, na.rm = T))
